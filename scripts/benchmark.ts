/**
 * Benchmark harness for Vextris effects and performance metrics.
...
/**
 * Simulated benchmark metrics for effects at different ranks
 */
interface BenchmarkMetrics {
  ...
  ghostCacheHitRate: number; // 0-1
  colorCacheHitRate: number; // 0-1
}

interface EffectBenchmark {
  effectName: string;
  enabled: boolean;
  results: BenchmarkMetrics[];
  totalTime: number;         // total time for benchmark
}

interface BenchmarkReport {
  timestamp: string;
  totalDuration: number;
  effects: EffectBenchmark[];
  summary: {
    avgFpsAllRanks: number;
    peakMemory: number;
    slowestFrame: number;
    fastestFrame: number;
  };
 ...
 */
function simulateFrameMetrics(
  effectName: string,
  rank: number,
  particleCount: number,
  poolingEnabled: boolean = true
): BenchmarkMetrics {
 ...
 */
function generateReport(effects: EffectBenchmark[]): BenchmarkReport {
